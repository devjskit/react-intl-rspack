...
import { type GlobPattern, getLocalNamespace, joinNamespace, normalizePattern } from "./namespace";
...
function getI18nFiles(patterns: string[], cwd: string, outputRel: string): string[] {
  ...
  return globSync(globPatterns, { cwd, nodir: true }).sort();
}
...
async function buildI18nEntries(files: string[], cwd: string, outputDirAbs: string, globalPrefix: string | undefined, debug: boolean): Promise<I18nEntry[]> {
  ...
  for (const relFile of files) {
    ...
    const namespace = joinNamespace(globalPrefix, getLocalNamespace(relFile, prefix));
    ...
    entries.push({ importVar, exportName, importPath, namespace });
  }
...
function buildI18nTypeAndFactory(languages: string[]): string {
  const languageLines = languages.length > 0 ? languages.map((lang) => `  ${JSON.stringify(lang)}?: Record<string, string>;`).join("\n") : "";
...
export const createMessages = (prefix: string, values: Record<string, string>): Messages => {
  const messages: Record<string, MessageDescriptor> = Object.entries(values).reduce(
    (acc, [key, value]) => {
      acc[key] = { id: prefix ? \`\${prefix}.\${key}\` : key, defaultMessage: value };
      return acc;
    },
...
function buildExportsForEntries(entries: I18nEntry[]): string {
  if (entries.length === 0) return "";
  return entries.map((entry) => `export const ${entry.exportName} = createI18n({ ...${entry.importVar}, $prefix: ${JSON.stringify(entry.namespace)} });`).join("\n");
}

export async function generateRootI18nFile(options: GenerateRootI18nFileOptions) {
  const { pattern, cwd, output, debug, languages, prefix } = options;
  ...
  if (files.length === 0) {
    ...
    const entries = await buildI18nEntries(files, cwd, outputDirAbs, prefix, debug);
    const header = `import { defineMessages, FormattedMessage } from "react-intl";`;
